...
> 报告含两大板块：**技术向 / B2B / 基础设施 AI** 与 **2C 消费向 AI**。
> 两端都要纳入考量，按分数择优；不要默认只从技术向板块选题（那正是过去系统性漏掉 2C 的原因）。
> 但所有机会仍须是 **AI 产品**（见 `config/lab-focus.md` 硬门槛）。

1. 客观提取信号：引用报告中的具体产品/趋势（标明这是报告事实），技术向与 2C 向都要看。
